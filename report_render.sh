...
url="https://binkuksouthstaging.blob.core.windows.net/qareports/pytest_report/binkweb/${bolb_name}"
...
az storage blob upload --account-name $(echo $BLOB_STORAGE_DSN | awk -F ';' '{print $2}' | sed 's/AccountName=//g') --container-name qareports --name "pytest_report/binkweb/$bolb_name" --file /tmp/reports/report.html --account-key $(echo $BLOB_STORAGE_DSN | awk -F ';' '{print $3}' | sed 's/AccountKey=//g') --auth-mode key
